<script setup lang="ts">
import TeamMember from '@/components/Teams/TeamMember.vue'


import {useTeamStore} from '@/stores/TeamStore.js'
let team = useTeamStore();
</script>

<template>
  <table class="w-full table-fixed border-spacing-2 border-separate">
    <thead>
    <th class="text-left px-6 py-2">Name</th>
    <th class="text-left px-6 py-2">Email</th>
    <th class="text-left px-6 py-2">Status</th>
    </thead>
    <TeamMember v-for="member in team.members" :name="member.name" :email="member.email" :status="member.status"/>

  </table>
  <p class="text-right text-gray-600 italic" v-show="team.members.length==team.spots">There are no ramaining team spots. Upgrade to add more.</p>
</template>

<style scoped></style>
